...
Transform options are attached to the most recent positional file argument. Put per-file
options such as `-t/--translate`, `-r/--rotate`, and `-s/--scale` after the input file they
should modify. For example, use `splat-transform input.lcc -r 90,0,0 output.lod-meta.json`,
not `splat-transform -r 90,0,0 input.lcc output.lod-meta.json`.
