...
const perFileOptions = new Set(Object.entries(cliOptionsConfig)
.filter(([, v]) => 'multiple' in v && v.multiple)
.map(([name]) => name)
);
...
const parseArguments = async () => {
    ...
    for (const t of tokens) {
        ...
        } else if (t.kind === 'option' && perFileOptions.has(t.name)) {
            throw new Error(
                `Option '--${t.name}' must appear after a positional input or output file. ` +
                `Example: splat-transform input.lcc --${t.name}${t.value !== undefined ? `=${t.value}` : ''} output.lod-meta.json`
            );
        }
    }
