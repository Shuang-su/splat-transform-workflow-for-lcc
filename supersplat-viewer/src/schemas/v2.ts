...
const validateBackgroundGradient = (data: unknown, path: string): BackgroundGradient => {
    const obj = assertObject(data, path);
    assertTuple3(obj.topColor, `${path}.topColor`);
    assertTuple3(obj.bottomColor, `${path}.bottomColor`);
    if (obj.horizonColor !== undefined) assertTuple3(obj.horizonColor, `${path}.horizonColor`);
    if (obj.horizonStop !== undefined) assertNumber(obj.horizonStop, `${path}.horizonStop`);
    if (obj.bottomStop !== undefined) assertNumber(obj.bottomStop, `${path}.bottomStop`);
    return data as BackgroundGradient;
};
...
const validateV2 = (data: unknown): ExperienceSettings => {
    ...
    if (bg.gradient !== undefined) validateBackgroundGradient(bg.gradient, 'settings.background.gradient');
...
export { validateV2 };
export type { AnimTrack, Camera, Annotation, PostEffectSettings, ExperienceSettings, BackgroundGradient };
