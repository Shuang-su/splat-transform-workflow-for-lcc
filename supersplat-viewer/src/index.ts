...
import { App } from './app';
import { hasContentRotationOverride, resolveContentRotation } from './content-transform';
import { observe } from './core/observe';
import { importSettings } from './settings';
import type { Config, Global } from './types';
import { initPoster, initUI } from './ui';
import { Viewer } from './viewer';
import { RotatedVoxelCollider, VoxelCollider } from './voxel-collider';
...
const loadGsplat = async (app: AppBase, config: Config, progressCallback: (progress: number) => void) => {
    ...
    return new Promise<Entity>((resolve, reject) => {
        asset.on('load', () => {
            ...
            const [x, y, z] = resolveContentRotation(config.contentRotation);
...
const main = async (canvas: HTMLCanvasElement, settingsJson: any, config: Config) => {
    ...
    const voxelLoad = config.voxelUrl &&
        VoxelCollider.load(config.voxelUrl)
            .then((collider) => {
                if (hasContentRotationOverride(config.contentRotation)) {
                    return new RotatedVoxelCollider(collider, resolveContentRotation(config.contentRotation));
                }
                return collider;
            })
