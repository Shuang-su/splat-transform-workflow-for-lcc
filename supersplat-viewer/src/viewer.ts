...
import type { BackgroundGradient, ExperienceSettings, PostEffectSettings } from './settings';
import type { Global } from './types';
import { VoxelCollider, type VoxelColliderLike } from './voxel-collider';
...
const gradientCss = (gradient: BackgroundGradient) => {
...
class Viewer {
    ...
    constructor(global: Global, gsplatLoad: Promise<Entity>, skyboxLoad: Promise<void> | undefined, voxelLoad: Promise<VoxelColliderLike | null> | undefined) {
        this.global = global;
    ...
    applyBackground(settings: ExperienceSettings) {
        ...
        const backgroundCss = gradient ? gradientCss(gradient) : toCssColor(background.color);

        document.documentElement.style.setProperty('--app-background', backgroundCss);
        document.documentElement.style.setProperty('--canvas-background', backgroundCss);
